...
class Trainer:
  ...
  def train(self):
    ...
    for batch in tqdm(self.train_data_loader):
      ...
      # mlm_input_ids = mlm_batch[0].to(self.device)
      # mlm_input_mask = mlm_batch[1].to(self.device)
  ...
  def fit(self,schedule=True,epochs=20,optim=None,report = False,name="saved",percentage=0.7):
    ...
    for epoch in range(epochs):
      ...
      with open(f"log\log-{self.name}{'-smart'if self.is_smart else ''}-{'boosting-' if self.extract else ''}{int(self.percentage*10)}_{10-int(self.percentage*10)}-3head.txt","a") as f:
        ...
        if valid_accs[1]>=0.8 and valid_accs[0]>=0.843 and valid_f1s[0]>=0.85 and valid_f1s[1]>=0.72:
            torch.save(self.bertcnn.state_dict(),f'models/{self.model_type}/Epoch-{epoch+1}-2-head-{self.model_type}-{"smart"if self.is_smart else ""}{"-boosting" if self.extract else ""}-{int(self.percentage*10)}_{10-int(self.percentage*10)}-3head.pt')
